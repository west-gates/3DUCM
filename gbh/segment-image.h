...
void sv_rgb(image<rgb> *im[], int num_frame, image<float> *sv_r[],
		image<float> *sv_g[], image<float> *sv_b[]){

	int width = im[0]->width();
	int height = im[0]->height();

	for (int i = 0; i < num_frame; i++) {
		sv_r[i] = new image<float>(width, height);
		sv_g[i] = new image<float>(width, height);
		sv_b[i] = new image<float>(width, height);
	}

	for (int i = 0; i < num_frame; i++) {
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				imRef(sv_r[i], x, y) = imRef(im[i], x, y).r;
				imRef(sv_g[i], x, y) = imRef(im[i], x, y).g;
				imRef(sv_b[i], x, y) = imRef(im[i], x, y).b;
			}
		}
	}
...
void segment_image(char *path, image<rgb> *im[], int num_frame, float c,
		float c_reg, int min_size, float sigma, int hie_num, image<rgb> *sv[]) {
	...
	image<float>** sv_r = new image<float>*[num_frame];
	image<float>** sv_g = new image<float>*[num_frame];
	image<float>** sv_b = new image<float>*[num_frame];
	smooth_images(im, num_frame, smooth_r, smooth_g, smooth_b, sigma);
	sv_rgb(sv,num_frame,sv_r,sv_g,sv_b);
	// ------------------------------------------------------------------
