...
int main(int argc, char **argv) {
	...
	// Initialize Parameters
	image<rgb>** sv = new image<rgb>*[frame_num];
	char filepath2[1024];


	// Read Frames
	for (int i = 0; i < frame_num; i++) {
		snprintf(filepath2, 1023, "%s/%05d.ppm", input_path2, i + 1);
		sv[i] = loadPPM(filepath2);
		printf("load --> %s\n", filepath2);
	}
	...
	// segmentation
	segment_image(output_path, images, frame_num, c, c_reg, min_size, sigma, hie_num,sv);
