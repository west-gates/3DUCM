...
void generate_edge(edge *e, image<float> *r_v, image<float> *g_v,
		...
		int z_u, image<float> *sv_rv, image<float> *sv_gv,image<float> *sv_bv,
		image<float> *sv_ru, image<float> *sv_gu,image<float> *sv_bu) {
	int width = r_v->width();
	...
	e->s=(imRef(sv_rv,x_v,y_v)==imRef(sv_ru,x_u,y_u))&&(imRef(sv_gv,x_v,y_v)==imRef(sv_gu,x_u,y_u))
			&&(imRef(sv_bv,x_v,y_v)==imRef(sv_bu,x_u,y_u));
...
void initialize_edges(edge *edges, int num_frame, int width, int height,
		...
		image<float> *smooth_b[], image<float> *sv_r[], image<float> *sv_g[],image<float> *sv_b[]) {
			...
			for (int x = 0; x < width; x++) {
				...
				if (x < width - 1) {
					generate_edge(&edges[num_edges], smooth_r[z], smooth_g[z],
							...
							x + 1, y, z, x, y, z ,sv_r[z],sv_g[z],sv_b[z],sv_r[z],sv_g[z],sv_b[z]);
					num_edges++;
				}
				if (y < height - 1) {
					generate_edge(&edges[num_edges], smooth_r[z], smooth_g[z],
							smooth_b[z], smooth_r[z], smooth_g[z], smooth_b[z],
							x, y + 1, z, x, y, z,sv_r[z],sv_g[z],sv_b[z],sv_r[z],sv_g[z],sv_b[z]);
					num_edges++;
				}
				if ((x < width - 1) && (y < height - 1)) {
					generate_edge(&edges[num_edges], smooth_r[z], smooth_g[z],
							smooth_b[z], smooth_r[z], smooth_g[z], smooth_b[z],
							x + 1, y + 1, z, x, y, z,sv_r[z],sv_g[z],sv_b[z],sv_r[z],sv_g[z],sv_b[z]);
					num_edges++;
				}
				if ((x < width - 1) && (y > 0)) {
					generate_edge(&edges[num_edges], smooth_r[z], smooth_g[z],
				...
				if (z > 0) {
					...
					generate_edge(&edges[num_edges], smooth_r[z - 1],
							...
							sv_r[z-1],sv_g[z-1],sv_b[z-1],sv_r[z],sv_g[z],sv_b[z]);
					num_edges++;
					...
					if (x > 0 && x < width - 1 && y > 0 && y < height - 1) {
						...
						generate_edge(&edges[num_edges], smooth_r[z - 1],
								...
								x, y, z,sv_r[z-1],sv_g[z-1],sv_b[z-1],sv_r[z],sv_g[z],sv_b[z]);
						num_edges++;
						// x, y - 1
						generate_edge(&edges[num_edges], smooth_r[z - 1],
								smooth_g[z - 1], smooth_b[z - 1], smooth_r[z],
								smooth_g[z], smooth_b[z], x, y - 1, z - 1, x, y,
								z,sv_r[z-1],sv_g[z-1],sv_b[z-1],sv_r[z],sv_g[z],sv_b[z]);
						num_edges++;
						// x + 1, y - 1
						generate_edge(&edges[num_edges], smooth_r[z - 1],
								smooth_g[z - 1], smooth_b[z - 1], smooth_r[z],
